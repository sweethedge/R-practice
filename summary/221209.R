...
# qplot은 deprecated 당했으니까..
qplot(거주지역, fill=거주지역, data=delirium) + ggtitle("거주지역별 치매환자")

# ggplot으로 바꾸기
c = as.data.frame(table(delirium$거주지역))
names(c) <- c('거주지역', '빈도')
ggplot(mapping = aes(x=거주지역, y=빈도, fill=거주지역), data = c) + geom_bar(stat = "identity") + ggtitle("거주지역별 치매환자") + theme(plot.title = element_text(hjust = 0.5))

# plot이라는 함수가 있다
# 이름이 다 안나온다
plot(table(delirium$거주지역))
barplot(table(delirium$거주지역))
